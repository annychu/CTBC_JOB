...
class CRF(nn.Module):
    def __init__(self, num_tags: int = 2, batch_first: bool = True) -> None:
        ...
        # 從 _compute_normalizer 中 next_score = broadcast_score + self.transitions + broadcast_emissions 可以看出
        # transitions[i][j] 表示從第i個tag 到第 j 個tag的分数
    ...
    def _computer_score(self,
        ...
        # 這裡是為了獲取每一個樣本最後一個詞的tag。
        # 每一个batch 的真实长度
        seq_ends = mask.long().sum(dim=0) - 1
        # 每個樣本最後一個詞的tag
        last_tags = tags[seq_ends, torch.arange(batch_size)]
        # 每一個樣本到最後一個詞的得分加上之前的score

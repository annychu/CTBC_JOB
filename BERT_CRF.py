...
class BertCrf(nn.Module):
    def __init__(self, config_name: str, model_name: str = None, num_tags: int = 2, batch_first: bool = True) -> None:
        ...
        # 模型的預訓練參數文件，如果没有，就不加載
        if model_name is not None:
            if not os.path.exists(model_name):
                raise ValueError(
                    "未找到模型預訓練參數文件 '{}'".format(model_name)
                )
            else:
    ...
    def forward(self, input_ids: torch.Tensor,
        ...
        # seq_len的维度，去头，是去掉了[CLS]，去尾巴有两种情况
        # 1、是 <pad> 2、[SEP]

        new_emissions = emissions[:, 1:-1]
        new_mask = attention_mask[:, 2:].bool()

        # 如果 tags 為 None，表示是一个預測的過程，不能求得loss,loss 直接為None
        if tags is None:
            loss = None
            pass
        else:

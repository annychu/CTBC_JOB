...
# 在這個case中只需要識別三個類型: LABELS = ["O", "B-LOC", "I-LOC"]。
...
def load_and_cache_example(args, tokenizer, processor, data_type):
    ...
    if os.path.exists(cached_features_file):
        ...
        features = crf_convert_examples_to_features(examples=examples, tokenizer=tokenizer,
                                                    max_length=args.max_seq_length, label_list=label_list)
...
def main():
    ...
    parser.add_argument("--data_dir", default=None, type=str, required=True,
                        help="資料集目錄，應有train.txt dev.txt")
    ...
    parser.add_argument("--max_seq_length", default=128, type=int,
                        help="BERT模型 input 最大長度，不可超過512")
    ...
    parser.add_argument("--warmup_steps", default=0, type=int,
                        help="讓學習率增加到1的步數，在warmup_steps後，再衰减到0")
    ...
    model = BertCrf(config_name=args.model_config, model_name=args.pre_train_model,
